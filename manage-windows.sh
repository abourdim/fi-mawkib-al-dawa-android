#!/usr/bin/env bash
# ============================================================================
#  fi Mawkib al-Dawa Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/22-fi-mawkib-al-dawa}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.fimawkibaldawa}"
AVD_NAME="${AVD_NAME:-fi_mawkib_al_dawa_test}"
...
cmd_test() {
  ...
  local out="/tmp/fi-mawkib-al-dawa-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   fi Mawkib al-Dawa Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
